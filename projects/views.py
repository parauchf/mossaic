...
@csrf_protect
def projectUsers(request,project_id):
...
def projectAddUser(request,project_id):
	if request.method=="POST":
...
def projectCommunities(request,project_id):
	if request.method=="GET":
		try:
...
def projectAddCommuntiy(request,project_id):
	if request.method=="POST":
		form = NewUserForm(request.POST)
...
def projectRiskMatrix(request,project_id):
	return render_to_response('projectAdministration.html',{'project_id':project_id})

def projectAdministration(request,project_id):
	return render_to_response('projectAdministration.html',{'project_id':project_id})
